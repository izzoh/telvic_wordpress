  ...
  function IsEmail(email) {
   ...
   if( !IsEmail( Email ) ) {
	...
	obj.find(".noticefailed").append("<img alt='loading' class='loading' src='"+themeurl+"/images/loading.gif' />");
	
	 jQuery.ajax({type:"POST",dataType:"json",url:ajaxurl,data:"Name="+Name+"&Email="+Email+"&Message="+Message+"&sendto="+sendto+"&action=onetone_contact",success:function(data){	if(data.error==0){obj.find(".noticefailed").addClass("noticesuccess").removeClass("noticefailed");obj.find(".noticesuccess").html(data.msg);}else{obj.find(".noticefailed").html(data.msg);	}jQuery('.loading').remove();},error:function(){obj.find(".noticefailed").html("Error.");obj.find('.loading').remove();}});});
...
jQuery(onetone_bigvideo.video_section_item).tubular(onetone_bigvideo.options);
 })